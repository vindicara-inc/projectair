...
<article class="max-w-3xl mx-auto px-6 py-16">
  ...
  <header class="mb-12">
    ...
    <h1 class="text-4xl sm:text-5xl font-bold tracking-tight leading-tight" style="color: var(--text-primary);">Run your first <code class="font-mono text-brand-purple">air trace</code> in 5 minutes</h1>
    <p class="text-lg mt-4" style="color: var(--text-muted);">From <code class="text-brand-purple bg-brand-purple/10 px-1.5 py-0.5 rounded text-sm">pip install projectair</code> to a signed forensic timeline of your LangChain agent. The <code class="text-brand-purple bg-brand-purple/10 px-1.5 py-0.5 rounded text-sm">air</code> CLI and <code class="text-brand-purple bg-brand-purple/10 px-1.5 py-0.5 rounded text-sm">airsdk</code> are MIT-licensed and open source today.</p>
  ...
  <h2 class="text-2xl font-bold mt-12 mb-4" style="color: var(--text-primary);">Why run <code class="font-mono text-brand-purple">air</code> before you have an incident</h2>
  ...
  <h2 class="text-2xl font-bold mt-12 mb-4" style="color: var(--text-primary);">Step 1: Install</h2>
  ...
  <h2 class="text-2xl font-bold mt-12 mb-4" style="color: var(--text-primary);">Step 2: Wire the AgDR callback into your agent</h2>
...
agent = AgentExecutor(callbacks=[handler], ...)`}</code></pre></div>
  ...
  <h2 class="text-2xl font-bold mt-12 mb-4" style="color: var(--text-primary);">Step 3: Run your agent once</h2>
  ...
  <div class="glass-panel rounded-xl p-8 my-12 text-center border-brand-red/20">
    <h3 class="text-xl font-bold mb-2" style="color: var(--text-primary);">Start the forensic record before the incident</h3>
    <p class="text-sm mb-6" style="color: var(--text-muted);"><code class="font-mono" style="color: var(--text-secondary);">air</code> and <code class="font-mono" style="color: var(--text-secondary);">airsdk</code> are MIT-licensed and open source. One <code class="font-mono" style="color: var(--text-secondary);">pip install</code> and you have a signed chain.</p>
  ...
  <h2 class="text-2xl font-bold mt-12 mb-4" style="color: var(--text-primary);">Step 4: Run <code class="font-mono text-brand-purple">air trace</code></h2>
...
[Export] forensic-report.json | forensic-report.pdf | forensic-report.siem`}</code></pre></div>
  ...
  <h2 class="text-2xl font-bold mt-12 mb-4" style="color: var(--text-primary);">Step 5: Export the forensic report</h2>
  ...
  <p class="leading-relaxed mt-4" style="color: var(--text-secondary);">
    <strong style="color: var(--text-primary);"><code class="text-brand-purple bg-brand-purple/10 px-1.5 py-0.5 rounded text-sm">forensic-report.json</code></strong>. The full decision chain with every AgDR entry, every hash, every signature, and every detection. This is the canonical evidence artifact. Any downstream tool can consume it.
  </p>

  <p class="leading-relaxed mt-4" style="color: var(--text-secondary);">
    <strong style="color: var(--text-primary);"><code class="text-brand-purple bg-brand-purple/10 px-1.5 py-0.5 rounded text-sm">forensic-report.pdf</code></strong>. A human-readable incident summary with the timeline, detections, and verification status. This is what legal, insurance, and executive stakeholders actually read.
  </p>
  ...
  <h2 class="text-2xl font-bold mt-12 mb-4" style="color: var(--text-primary);">What comes next</h2>
  ...
  <!-- End-of-article CTA -->
  <div class="glass-panel rounded-xl p-8 my-12 text-center border-brand-red/20">
    <h3 class="text-xl font-bold mb-2" style="color: var(--text-primary);">Your next incident is already on its way.</h3>
    <p class="text-sm mb-6" style="color: var(--text-muted);">Make sure you can prove what happened. <code class="font-mono" style="color: var(--text-secondary);">pip install projectair</code> and the record starts now.</p>
    <div class="flex flex-col sm:flex-row items-center justify-center gap-3">
      <a href="https://github.com/vindicara-inc/projectair?utm_source=blog&utm_medium=cta&utm_campaign=secure-ai-agents-5-minutes" class="btn-primary text-sm px-6 py-3">View on GitHub</a>
      <a href="https://vindicara.io/#how-it-works?utm_source=blog&utm_medium=cta&utm_campaign=secure-ai-agents-5-minutes" class="btn-secondary text-sm px-6 py-3">How AIR works</a>
    </div>
