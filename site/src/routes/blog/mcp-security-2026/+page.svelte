...
<article class="max-w-3xl mx-auto px-6 py-16">
  ...
  <header class="mb-12">
    ...
    <h1 class="text-4xl sm:text-5xl font-bold tracking-tight leading-tight" style="color: var(--text-primary);">The State of MCP Security in 2026</h1>
    <p class="text-lg mt-4" style="color: var(--text-muted);">92% of MCP servers lack proper OAuth. We scanned real configurations and found critical vulnerabilities across authentication, authorization, and resource management.</p>
  ...
  <h2 class="text-2xl font-bold mt-12 mb-4" style="color: var(--text-primary);">The MCP adoption explosion</h2>
  ...
  <h2 class="text-2xl font-bold mt-12 mb-4" style="color: var(--text-primary);">The 92% problem</h2>
  ...
  <h2 class="text-2xl font-bold mt-12 mb-4" style="color: var(--text-primary);">What a vulnerable MCP server looks like</h2>
  ...
  <div class="glass-panel rounded-xl p-8 my-12 text-center border-brand-red/20">
    <h3 class="text-xl font-bold mb-2" style="color: var(--text-primary);">Reconstruct the next MCP incident</h3>
    <p class="text-sm mb-6" style="color: var(--text-muted);">The <code class="font-mono" style="color: var(--text-secondary);">air</code> CLI is open source. Ingest any agent trace and get a signed forensic timeline in seconds.</p>
  ...
  <h2 class="text-2xl font-bold mt-12 mb-4" style="color: var(--text-primary);">Common vulnerability patterns</h2>
  ...
  <h2 class="text-2xl font-bold mt-12 mb-4" style="color: var(--text-primary);">What to do about it</h2>
  ...
  <p class="leading-relaxed mt-4" style="color: var(--text-secondary);">
    <strong style="color: var(--text-primary);">1. Implement OAuth 2.0 with PKCE for all MCP connections.</strong> This is the single highest-impact fix. Every MCP server must authenticate its clients. PKCE (Proof Key for Code Exchange) prevents authorization code interception attacks, which is critical in environments where agents may be running in shared infrastructure.
  </p>

  <p class="leading-relaxed mt-4" style="color: var(--text-secondary);">
    <strong style="color: var(--text-primary);">2. Remove or sandbox dangerous tools.</strong> Tools like <code class="text-brand-cyan bg-brand-cyan/10 px-1.5 py-0.5 rounded text-sm">shell_exec</code> should never be exposed without sandboxing. If shell access is genuinely required, run it inside a restricted container with no network access, no filesystem writes outside a designated directory, and a hard timeout on execution.
  </p>

  <p class="leading-relaxed mt-4" style="color: var(--text-secondary);">
    <strong style="color: var(--text-primary);">3. Add row-level access controls to data mutation tools.</strong> Every tool that modifies data needs scoping. An agent should only be able to delete, update, or create records within the boundaries of its assigned task and tenant.
  </p>

  <p class="leading-relaxed mt-4" style="color: var(--text-secondary);">
    <strong style="color: var(--text-primary);">4. Restrict file access to an allowlist of safe paths.</strong> Never give an agent access to the entire filesystem. Define an explicit allowlist of directories and file patterns, and reject any path that does not match.
  </p>

  <p class="leading-relaxed mt-4" style="color: var(--text-secondary);">
    <strong style="color: var(--text-primary);">5. Implement server-side rate limiting.</strong> Set per-client request limits and return <code class="text-brand-cyan bg-brand-cyan/10 px-1.5 py-0.5 rounded text-sm">HTTP 429</code> responses when thresholds are exceeded. This protects against both accidental runaway agents and deliberate abuse.
  </p>
...
for finding in report.findings:
  ...
  <h2 class="text-2xl font-bold mt-12 mb-4" style="color: var(--text-primary);">Scanning is only half the story</h2>
  ...
  <!-- End-of-article CTA -->
  <div class="glass-panel rounded-xl p-8 my-12 text-center border-brand-red/20">
    <h3 class="text-xl font-bold mb-2" style="color: var(--text-primary);">When prevention fails, reconstruction wins</h3>
    <p class="text-sm mb-6" style="color: var(--text-muted);">Project AIR is the forensic and incident response layer for AI agents. Open source today.</p>
    <div class="flex flex-col sm:flex-row items-center justify-center gap-3">
      <a href="https://github.com/vindicara-inc/projectair?utm_source=blog&utm_medium=cta&utm_campaign=mcp-security-2026" class="btn-primary text-sm px-6 py-3">View on GitHub</a>
      <a href="https://vindicara.io/#how-it-works?utm_source=blog&utm_medium=cta&utm_campaign=mcp-security-2026" class="btn-secondary text-sm px-6 py-3">How AIR works</a>
    </div>
