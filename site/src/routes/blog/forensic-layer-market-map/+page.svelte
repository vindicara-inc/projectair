...
<article class="max-w-3xl mx-auto px-6 py-16">
  <header class="mb-12">
    ...
    <h1 class="text-4xl sm:text-5xl font-bold tracking-tight leading-tight" style="color: var(--text-primary);">What happens after an AI agent does something it shouldn't?</h1>
    <p class="text-lg mt-4" style="color: var(--text-muted);">A map of AI agent security tooling, and the layer most teams don't realize they're missing.</p>
  ...
  <h2 class="text-2xl font-bold mt-12 mb-4" style="color: var(--text-primary);">Three layers</h2>
  ...
  <ol class="list-decimal pl-6 mt-4 space-y-2 leading-relaxed" style="color: var(--text-secondary);">
    <li><strong style="color: var(--text-primary);">Pre-incident.</strong> Prevention. Block prompts, gate tool calls, scan inputs, enforce output schemas.</li>
    <li><strong style="color: var(--text-primary);">During-incident.</strong> Observability. Traces, evals, dashboards, latency, prompt versioning, cost.</li>
    <li><strong style="color: var(--text-primary);">Post-incident.</strong> Forensics. Reconstruct what happened. Sign it. Verify it against an authorization scope. Hand it to legal, regulators, or insurers in a form that holds up.</li>
  </ol>
  ...
  <h2 class="text-2xl font-bold mt-12 mb-4" style="color: var(--text-primary);">Pre-incident: runtime defense</h2>
  ...
  <p class="leading-relaxed mt-4" style="color: var(--text-secondary);">
    The names: <strong style="color: var(--text-primary);">Lakera Guard</strong> sits in front of LLM endpoints and classifies prompt-injection attempts. <strong style="color: var(--text-primary);">Protect AI Guardian</strong> does runtime model scanning and policy enforcement. <strong style="color: var(--text-primary);">HiddenLayer</strong> focuses on ML supply-chain risk and runtime detection. <strong style="color: var(--text-primary);">NeMo Guardrails</strong> (NVIDIA) is a declarative input/output gating framework you wire into LangChain or LlamaIndex agents. <strong style="color: var(--text-primary);">Guardrails AI</strong> enforces structured output schemas and policy violations on LLM responses. <strong style="color: var(--text-primary);">CalypsoAI</strong> and <strong style="color: var(--text-primary);">Robust Intelligence</strong> sit adjacent, broadening into governance and red-teaming respectively.
  </p>
  ...
  <h2 class="text-2xl font-bold mt-12 mb-4" style="color: var(--text-primary);">During-incident: observability</h2>
  ...
  <p class="leading-relaxed mt-4" style="color: var(--text-secondary);">
    The names: <strong style="color: var(--text-primary);">LangSmith</strong> (LangChain's hosted observability), <strong style="color: var(--text-primary);">Arize Phoenix</strong> (open source agent traces with LLM-eval primitives), <strong style="color: var(--text-primary);">Langfuse</strong> (open source LLM observability and prompt management), <strong style="color: var(--text-primary);">Helicone</strong> (LLM proxy with built-in tracing), <strong style="color: var(--text-primary);">Weights & Biases Weave</strong> (the W&B extension into LLM observability), <strong style="color: var(--text-primary);">Datadog LLM Observability</strong> (the obvious enterprise default for shops already on Datadog), <strong style="color: var(--text-primary);">New Relic AI Monitoring</strong> (same play for the New Relic shops), <strong style="color: var(--text-primary);">Honeycomb</strong> for teams using it for general distributed tracing.
  </p>
  ...
  <h2 class="text-2xl font-bold mt-12 mb-4" style="color: var(--text-primary);">Post-incident: forensics</h2>
  ...
  <p class="leading-relaxed mt-4" style="color: var(--text-secondary);">
    You can pipe LLM traces into general SIEM tooling. <strong style="color: var(--text-primary);">Splunk</strong>, <strong style="color: var(--text-primary);">Datadog Cloud SIEM</strong>, <strong style="color: var(--text-primary);">Elastic Security</strong>, <strong style="color: var(--text-primary);">Sumo Logic</strong>, all of them ingest application logs and apply detection rules. They are mature, they are battle-tested, they have integrations with everything. What they do not do is model agent semantics at the schema level. They treat an agent trace as one more application log stream. The concepts that matter for agent forensics, capsules, tool authorization scope, signed envelope chains, multi-agent coordination, are not first-class in any general SIEM today. You can build them on top, the same way you can build anything on top of Splunk, but you are doing schema work the SIEM does not help with.
  </p>

  <p class="leading-relaxed mt-4" style="color: var(--text-secondary);">
    Below the SIEM layer is <strong style="color: var(--text-primary);">AWS CloudTrail</strong>, <strong style="color: var(--text-primary);">GuardDuty</strong>, and equivalents from GCP and Azure. These are infrastructure-level. They tell you a Lambda invoked another Lambda. They are agent-blind by design.
  </p>
  ...
  <p class="leading-relaxed mt-4" style="color: var(--text-secondary);">
    First, <strong style="color: var(--text-primary);">AgDR</strong> (the Agent Data Record format), originated by accountability.ai and the work of Mahmoud Mohamed Anwar (me2resh). AgDR is an open schema specification for signed agent records. It defines a record envelope, a payload structure, a signing primitive, and a verification model designed specifically for agent forensics. It is the closest thing the field has to a canonical schema for this layer. We adopted it.
  </p>

  <p class="leading-relaxed mt-4" style="color: var(--text-secondary);">
    Second, <strong style="color: var(--text-primary);">Project AIR</strong>, the open source SDK and CLI we ship as <code class="font-mono text-brand-red">projectair</code> on PyPI under MIT. AIR is the reference implementation that produces AgDR-format Signed Intent Capsules from running agents. The capsule is signed with Ed25519 over a BLAKE3-hashed payload, chained to the previous capsule in the session, verifiable with a published key, and emitted in real time as the agent runs. AIR ships 16 detectors: 10 covering the OWASP Top 10 for Agentic Applications (ASI01 through ASI10), 3 covering the OWASP Top 10 for LLM Applications categories most relevant to agent runtimes (LLM01 prompt injection, LLM06 sensitive information disclosure, LLM04 model denial of service), and 3 AIR-native detectors (forensic chain integrity, plus NemoGuard safety and corroboration). 14 run offline out of the box; the 2 NemoGuard detectors activate with an NVIDIA NemoGuard NIM. ASI10 is implemented as Zero-Trust behavioral-scope enforcement against an operator-declared scope, which is what the OWASP spec mitigation describes. It is not anomaly detection. The learned-baseline anomaly variant is on the roadmap.
  </p>
  ...
  <h2 class="text-2xl font-bold mt-12 mb-4" style="color: var(--text-primary);">Why this matters</h2>
  ...
  <h2 class="text-2xl font-bold mt-12 mb-4" style="color: var(--text-primary);">How the layers compose</h2>
  ...
  <h2 class="text-2xl font-bold mt-12 mb-4" style="color: var(--text-primary);">What we shipped</h2>
