<script lang="ts">
  ...
  const shareUrl = 'https://vindicara.io/blog/trustworthy-agents-forensic-evidence';
  const shareTitle = 'Implementing Trustworthy Agents: A Forensic Evidence Layer for Production';
  const shareDesc = "Anthropic's April 9 paper on trustworthy agents names three ecosystem gaps. Project AIR is our answer to evidence sharing and open standards.";
</script>
...
<article class="max-w-3xl mx-auto px-6 py-16">
  <header class="mb-12">
    ...
    <h1 class="text-4xl sm:text-5xl font-bold tracking-tight leading-tight" style="color: var(--text-primary);">Implementing Trustworthy Agents: A Forensic Evidence Layer for Production</h1>
    <p class="text-lg mt-4" style="color: var(--text-muted);">Anthropic's April 9 paper on trustworthy agents names three places the ecosystem must step up. Project AIR is our answer to evidence sharing and open standards, and a concrete contribution to a problem no single company can solve alone.</p>
  ...
  <p class="leading-relaxed mt-4" style="color: var(--text-secondary);">
    On April 9, Anthropic published <em>Trustworthy Agents in Practice</em>. It is the most honest thing I have read from a frontier lab about where agent security actually stands.
  </p>
  ...
  <blockquote class="border-l-4 border-brand-red/40 pl-6 my-8 italic space-y-3" style="color: var(--text-secondary);">
  ...
  <p class="leading-relaxed mt-4" style="color: var(--text-secondary);">
    Anthropic names three places where the ecosystem needs to step up: <strong style="color: var(--text-primary);">shared benchmarks, evidence sharing, and open standards.</strong> Project AIR is our answer to the second one, and a down payment on the third.
  </p>

  <h2 class="text-2xl font-bold mt-12 mb-4" style="color: var(--text-primary);">The four components, and where the gap is</h2>

  <p class="leading-relaxed mt-4" style="color: var(--text-secondary);">
    The paper identifies four components that determine how any agent behaves: the <strong style="color: var(--text-primary);">model</strong>, the <strong style="color: var(--text-primary);">harness</strong> (instructions and guardrails), the <strong style="color: var(--text-primary);">tools</strong> it can call, and the <strong style="color: var(--text-primary);">environment</strong> it runs in. Anthropic is upfront that most industry conversation centers on the model, "and understandably so," but that agent behavior depends on all four layers working together.
  </p>
  ...
  <p class="leading-relaxed mt-4" style="color: var(--text-secondary);">
    What none of those layers produce is a <strong style="color: var(--text-primary);">signed, classified, exportable record of what the agent actually did when something went wrong.</strong>
  </p>
  ...
  <h2 class="text-2xl font-bold mt-12 mb-4" style="color: var(--text-primary);">What AIR is</h2>
  ...
  <p class="leading-relaxed mt-4" style="color: var(--text-secondary);">
    <strong style="color: var(--text-primary);"><code class="text-brand-red bg-brand-red/10 px-1.5 py-0.5 rounded text-sm">airsdk</code></strong>: the Python SDK. MIT-licensed. Drop-in LangChain callback handler. Every agent decision written as an <strong style="color: var(--text-primary);">AgDR record</strong> (AI Decision Record), with BLAKE3 content hashing, Ed25519 signatures, and UUIDv7 ordering, forward-chained for tamper evidence.
  </p>

  <p class="leading-relaxed mt-4" style="color: var(--text-secondary);">
    <strong style="color: var(--text-primary);">AIR Cloud</strong>: hosted incident response. Real-time dashboards, SIEM integrations, compliance exports, insurance-ready evidence packs. Coming soon.
  </p>
...
agent = AgentExecutor(callbacks=[handler])`}</code></pre></div>
  ...
  <h2 class="text-2xl font-bold mt-12 mb-4" style="color: var(--text-primary);">Why this matters for the four-layer framework</h2>
  ...
  <p class="leading-relaxed mt-4" style="color: var(--text-secondary);">
    AIR operates across the <strong style="color: var(--text-primary);">tools</strong> and <strong style="color: var(--text-primary);">environment</strong> layers. When an agent calls a tool it should not have had access to, AIR signs and classifies that call. When an environment transitions, a new MCP server registered, a permission escalated, a file written to a sensitive path, AIR records the transition. When the agent completes a task or fails one, AIR produces evidence you can verify.
  </p>

  <p class="leading-relaxed mt-4" style="color: var(--text-secondary);">
    In the framework's language: AIR is how the <strong style="color: var(--text-primary);">transparency</strong> principle gets made real after the fact. You can claim transparency. Or you can prove it.
  </p>

  <div class="glass-panel rounded-xl p-8 my-12 text-center border-brand-red/20">
    <h3 class="text-xl font-bold mb-2" style="color: var(--text-primary);">The forensic evidence layer is open source today</h3>
    <p class="text-sm mb-6" style="color: var(--text-muted);"><code class="font-mono" style="color: var(--text-secondary);">air</code> and <code class="font-mono" style="color: var(--text-secondary);">airsdk</code> are MIT-licensed. <code class="font-mono" style="color: var(--text-secondary);">pip install projectair</code> and the signed chain starts now.</p>
    <div class="flex flex-col sm:flex-row items-center justify-center gap-3">
      <a href="https://github.com/vindicara-inc/projectair?utm_source=blog&utm_medium=cta&utm_campaign=trustworthy-agents" class="btn-primary text-sm px-6 py-3">View on GitHub</a>
      <a href="https://vindicara.io/#how-it-works?utm_source=blog&utm_medium=cta&utm_campaign=trustworthy-agents" class="btn-secondary text-sm px-6 py-3">How AIR works</a>
    </div>
  </div>

  <h2 class="text-2xl font-bold mt-12 mb-4" style="color: var(--text-primary);">Why OWASP ASI, not a vendor taxonomy</h2>
  ...
  <h2 class="text-2xl font-bold mt-12 mb-4" style="color: var(--text-primary);">Real incidents, real mappings</h2>
  ...
  <p class="leading-relaxed mt-4" style="color: var(--text-secondary);">
    The <a href="/" class="text-brand-red hover:text-brand-red/80 underline">incidents table on vindicara.io</a> walks through what AIR's detection signatures would have caught at the step the breach actually happened. Every mapping is against the OWASP 2026 taxonomy.
  </p>

  <h2 class="text-2xl font-bold mt-12 mb-4" style="color: var(--text-primary);">What we are doing, and what we are asking for</h2>

  <p class="leading-relaxed mt-4" style="color: var(--text-secondary);">
    <code class="text-brand-red bg-brand-red/10 px-1.5 py-0.5 rounded text-sm">projectair</code> ships on PyPI today. The MIT SDK and CLI are live. The design partner program for AIR Cloud opens May 4: three production LangChain deployments, sixty days of feedback, preferred pricing in return.
  </p>
  ...
  <p class="leading-relaxed mt-4" style="color: var(--text-secondary);">
    AIR contributes to <strong style="color: var(--text-primary);">evidence sharing</strong> and <strong style="color: var(--text-primary);">open standards</strong>. We would like to work with labs, standards bodies, and infrastructure providers on making the evidence format interoperable, so an incident detected in a LangChain agent on AWS produces the same AgDR record as one detected in a CrewAI agent on Azure, verifiable by any downstream consumer.
  </p>

  <p class="leading-relaxed mt-4" style="color: var(--text-secondary);">
    If you are running agents in production, try it. <code class="text-brand-red bg-brand-red/10 px-1.5 py-0.5 rounded text-sm">pip install projectair</code> and run <code class="text-brand-red bg-brand-red/10 px-1.5 py-0.5 rounded text-sm">air demo</code>. You will see exactly what your current tooling does not capture.
  </p>
  ...
  <div class="glass-panel rounded-xl p-8 my-12 text-center border-brand-red/20">
    <h3 class="text-xl font-bold mb-2" style="color: var(--text-primary);">Build the evidence layer with us.</h3>
    <p class="text-sm mb-6" style="color: var(--text-muted);"><code class="font-mono" style="color: var(--text-secondary);">pip install projectair</code> to start. Open issues, PRs, and threat-model contributions welcome on GitHub.</p>
    <div class="flex flex-col sm:flex-row items-center justify-center gap-3">
      <a href="https://github.com/vindicara-inc/projectair?utm_source=blog&utm_medium=cta&utm_campaign=trustworthy-agents-end" class="btn-primary text-sm px-6 py-3">View on GitHub</a>
      <a href="https://vindicara.io/#how-it-works?utm_source=blog&utm_medium=cta&utm_campaign=trustworthy-agents-end" class="btn-secondary text-sm px-6 py-3">How AIR works</a>
    </div>
  </div>

  <div class="mt-12 pt-8 text-sm space-y-4" style="border-top: 1px solid var(--border-subtle); color: var(--text-muted);">
    <p>
      <strong style="color: var(--text-primary);">Kevin Minn</strong> is the founder of Vindicara. AIR is MIT-licensed at <a href="https://vindicara.io" class="text-brand-red hover:text-brand-red/80 underline">vindicara.io</a>. The CLI and SDK are on PyPI: <code class="font-mono" style="color: var(--text-secondary);">pip install projectair</code>. Source at <a href="https://github.com/vindicara-inc/projectair" class="text-brand-red hover:text-brand-red/80 underline">github.com/vindicara-inc/projectair</a>.
    </p>
    <div>
      <p class="font-semibold mb-2" style="color: var(--text-primary);">References</p>
      <ul class="list-disc list-inside space-y-1">
        <li>Anthropic, <em>Trustworthy Agents in Practice</em> (April 9, 2026): <a href="https://www.anthropic.com/research/trustworthy-agents" class="text-brand-red hover:text-brand-red/80 underline">anthropic.com/research/trustworthy-agents</a></li>
        <li>OWASP Top 10 for Agentic Applications 2026 (ASI01 through ASI10)</li>
        <li>Model Context Protocol (Linux Foundation Agentic AI Foundation): <a href="https://modelcontextprotocol.io" class="text-brand-red hover:text-brand-red/80 underline">modelcontextprotocol.io</a></li>
      </ul>
    </div>
  </div>
  ...
  <div class="mt-16 pt-8" style="border-top: 1px solid var(--border-subtle);">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
      <a href="/blog/secure-ai-agents-5-minutes" class="glass-panel rounded-lg p-4 hover:border-brand-purple/30 transition-colors block">
        ...
        <p class="text-sm font-medium mt-1" style="color: var(--text-primary);">Run your first <code class="font-mono">air trace</code> in 5 minutes</p>
      </a>
      <a href="/blog/eu-ai-act-article-72-guide" class="glass-panel rounded-lg p-4 hover:border-green-500/30 transition-colors block">
        ...
        <p class="text-sm font-medium mt-1" style="color: var(--text-primary);">EU AI Act Article 72: A Developer's Guide</p>
      </a>
    </div>
  </div>
