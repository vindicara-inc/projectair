<script lang="ts">
  let { on = false, accent = 'teal', disabled = false, onchange = (_v: boolean) => {} } = $props();
...
<style>
  ...
  .sw.on { background: linear-gradient(90deg, #48e6a4, #13c08a); }
  .sw.on::after { left: 18px; }
  .sw.vio.on { background: linear-gradient(90deg, #9b6bff, #6db5ff); }
