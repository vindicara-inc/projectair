<script lang="ts">
  import { locked, beginAuth0Login, unlock, authError } from '$lib/console/stores/session';
  ...
  $effect(() => authError.subscribe((v) => { err = v; }));
...
{#if isLocked}
  <div class="lockwrap">
    ...
    {/if}
    {#if phase === 'login' && !isLive}
      <div class="lock">
        <div class="lcard glass hud k">
          <div class="llogo"><span class="dot"></span><h2>Project&nbsp;<span class="air">AIR</span></h2><span class="ltag">Flightdeck</span></div>
          <div class="lsub">Flightdeck session locked. Re-authenticate to resume agents.</div>
    ...
    {#if phase === 'login' && isLive}
      <div class="lock">
        <div class="lcard glass hud k">
          <div class="llogo"><span class="dot"></span><h2>Project&nbsp;<span class="air">AIR</span></h2><span class="ltag">Flightdeck</span></div>
          <div class="lsub">Redirecting to Auth0 Universal Login…</div>
          {#if err}
            <div class="lerr">{err}</div>
            <button class="lbtn" onclick={doUnlock}>Retry sign-in</button>
          {/if}
        </div>
      </div>
    {/if}
...
<style>
  ...
  .ltag { font-family: var(--mono); font-size: 9px; letter-spacing: .16em; text-transform: uppercase; color: var(--faint); margin-left: 4px; }
  .lsub { font-size: 12.5px; color: var(--muted); margin: 7px 0 22px; }
  .lerr { font-size: 12px; color: #ffc4a3; margin-bottom: 14px; line-height: 1.45; }
