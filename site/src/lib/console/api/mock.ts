...
export class MockClient implements ApiClient {
  ...
  async getReadiness(): Promise<ReadinessData> {
    ...
    return {
      ...
      compliance: [
        ...
        { framework: 'SB 53', detail: 'disclosure', pct: 90, state: 'good' }
      ]
    };
  }
  ...
  async getInsurance(): Promise<InsuranceData> {
    ...
    return {
      ...
      consents: [
        { carrier: 'coalition', authorizer: 'dr.okafor authorized Coalition', detail: 'Scope: posture feed + evidence on incident. Passkey-signed, expires in 30 days.', status: 'active' },
        { carrier: 'vouch', authorizer: 'Pending: Vouch (broker request)', detail: 'Carrier requested read access. Waiting on buyer approval.', status: 'pending' }
      ],
      connectedActive: 1,
      connectedPending: 1,
    ...
    return {
      ...
      sections: [
        ...
        { title: 'Compliance & billing', accent: 'var(--amber)', rows: [
          { label: 'Frameworks', kind: 'value', value: 'EU AI Act · HIPAA · NIST · SB 53' },
  ...
  async connectPlugin(_pluginId: string) { await delay(120); }
}
