...
{:then d}
  ...
  <div class="rules">
    <Panel reveal klass="rfiles">
      ...
      {/each}
      <button class="rfile new" onclick={() => rulesNote = 'Ruleset authoring persists through the AIR policy registry API (shipping next). View and inspect are live.'}><span>+ new ruleset</span></button>
    </Panel>

    <Panel reveal delay={0.06} klass="reditor">
      <div class="rebar"><span class="fn">{current.name}</span><span class="lock-note">{current.layerNote}</span><button class="savebtn" onclick={() => rulesNote = 'Save & version writes to the workspace policy store (read-only in this release).'}>Save &amp; version</button></div>
      {#if rulesNote}<div class="rulesNote">{rulesNote}</div>{/if}
...
<style>
  ...
  .rulesNote { padding: 10px 16px; font-size: 11.5px; color: #ffd49a; border-bottom: 1px solid var(--hair); }
