<script lang="ts">
  ...
  async function connect(id: string) {
    ...
    pluginError = null;
    try {
      await api.connectPlugin(id);
      load = api.getPlugins();
    } catch (err) {
      pluginError = err instanceof Error ? err.message : 'Plugin connect failed.';
    } finally {
      connecting = null;
    }
  }
...
{#snippet card(p: Plugin)}
  <Panel klass="pcard">
    ...
    <div class="prow">
      ...
      {#if p.status !== 'connected'}<Toggle on={false} disabled={connecting === p.id} onchange={() => connect(p.id)} />{/if}
    </div>
  </Panel>
{/snippet}
...
{:then d}
  {#if pluginError}<Panel><StateBlock kind="error" message={pluginError} /></Panel>{/if}
  <div class="pgrid">
  ...
  <div class="vhead">
    ...
    <span class="sp"></span>
    <button class="btn ok" onclick={() => goto('/dashboard/insurance/')}>Open insurance API &rarr;</button>
  </div>
  <div class="pgrid">
    {#each d.insurance as p}{@render card(p)}{/each}
    <button class="panel glass hud pcard ghost" onclick={() => goto('/dashboard/insurance/')}>
