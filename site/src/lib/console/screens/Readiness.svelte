<script lang="ts">
  ...
  const ring = (pct: number) => 100 - pct;

  function exportReport() {
    exportNote = isLive
      ? 'Readiness PDF export uses /v1/compliance/reports on the API. Wire your system_id in Settings before exporting.'
      : 'Export is available in live mode against the compliance reporter.';
  }
</script>
...
{:then d}
  <Panel reveal klass="score">
    ...
    <div class="acts">
      <button class="authbtn ok" onclick={exportReport}><span class="key"></span>Export readiness report</button>
      <button class="btn" onclick={exportReport}>Share with buyer</button>
    </div>
    {#if exportNote}<div class="exportNote">{exportNote}</div>{/if}
  </Panel>
...
<style>
  ...
  .exportNote { width: 100%; font-size: 11.5px; color: #ffd49a; margin-top: 8px; }
