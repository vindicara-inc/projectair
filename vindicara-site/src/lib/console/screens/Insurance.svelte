<script lang="ts">
  ...
  async function toggle(label: string, on: boolean) { await api.setTransport(label, on); load = api.getInsurance(); }
  async function revoke(carrier: string) { await api.revokeConsent(carrier); load = api.getInsurance(); }
</script>
    ...
    <Panel reveal>
      ...
      {#each d.transport as t}
        <div class="srow">
          ...
          <Toggle on={t.on} disabled={t.locked} onchange={(v: boolean) => toggle(t.label, v)} />
        </div>
      {/each}
    </Panel>
    ...
    <Panel reveal delay={0.06}>
      ...
      {#each d.consents as c}
        <div class="rdrow">
          ...
          <button class="rbtn" onclick={() => revoke(c.authorizer)}>{c.status === 'active' ? 'Revoke' : 'Review'}</button>
        </div>
      {/each}
    </Panel>
  </div>
  ...
  <Panel reveal delay={0.12} klass="why">
    <div class="whygrid">
      ...
      <div>
        ...
        <button class="verbtn">Send evidence pack to carrier</button>
      </div>
    </div>
  </Panel>
{:catch err}
