...
- Framework integrations for CrewAI / AutoGen
- AIR Cloud (hosted ingestion + dashboard backing the Team tier)

### Framework integrations shipped

LangChain (`AIRCallbackHandler`), OpenAI (`instrument_openai`), Anthropic (`instrument_anthropic`), LlamaIndex (`instrument_llamaindex`, shipped 0.3.1). CrewAI and AutoGen are on the roadmap.
...
- `packages/projectair/src/airsdk/integrations/` — `openai.py` (`instrument_openai`), `anthropic.py` (`instrument_anthropic`), `llamaindex.py` (`instrument_llamaindex`, transparent proxy over any `llama_index.core.llms.LLM` subclass). LangChain lives in `callback.py` (historical; may relocate).
